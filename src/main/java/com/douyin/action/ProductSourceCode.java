...
import com.douyin.service.GetProductCatePropertyV2;
import com.douyin.service.GetProductComponentTemplate;
import com.douyin.service.GetProductShopCategory;
...
import static com.douyin.utils.StringUtils.getCharacterCount;

// Press Shift twice to open the Search Everywhere dialog and type `show whitespaces`,
// then press Enter. You can now see whitespace characters in your code.
//平台商品链接OK－－商品详情OK－建立数据库－保存商品详情到数据库－通过抖音图片识别获取标题NOT－上传到抖店草稿箱－人工审核－抖店批量展示商品
//chrome的自动更新功能关闭
//直接读取商品链接，再保存到抖店
public class ProductSourceCode {
    public static void main(String[] args) {
        ...
        String accessToken = "78e22d69-595e-49c0-87a4-6bf6e48be0b3"; // 替换成你的Access Token
        String apiKey = "bolin20230727";

        String filePath = "E:\\e3huiman\\T恤10-150.txt";
        //分词器
        String dictionaryPath = "E:\\projects\\douyinGather\\library\\custom_dictionary.txt";
        String titlePath = "E:\\projects\\douyinGather\\library\\title.txt";
                ...
                if (data.has("data")) {
                    ...
                    for (int i = 0; i < properties.length(); i++) {
                        ...
                        if (propertyName.equals("品牌")) {
                            ...
                            if (!matchFound && count == 0) {
                                attributes.put("value", Integer.parseInt(seasons.getJSONObject(0).get("value").toString()));
                                attributes.put("name", seasons.getJSONObject(0).get("name"));
                                attributes.put("diy_type", 0);
                                attributesArray.put(attributes);
                                commit = false;
                                count++;
                            ...
                            if ((!matchFound) && count == 0) {
                                attributes.put("value", Integer.parseInt(shiyongduixiangs.getJSONObject(0).get("value").toString()));
                                attributes.put("name", shiyongduixiangs.getJSONObject(0).get("name"));
                                attributes.put("diy_type", 0);
                                attributesArray.put(attributes);
                                commit = false;
                                count++;
                            ...
                            if (!fabricTextElements.isEmpty()) {
                                ...
                                System.out.println("Fabric Text: " + fabricText);
                            } else {
                                // 元素不存在
                                System.out.println("Fabric Text element not found.");
                            }
                            ...
                            if (!matchFound && count == 0) {
                                attributes.put("value", Integer.parseInt(fabrics.getJSONObject(0).get("value").toString()));
                                attributes.put("name", fabrics.getJSONObject(0).get("name"));
                                attributes.put("diy_type", 0);
                                attributesArray.put(attributes);
                                commit = false;
                                count++;
                            ...
                            if (!matchFound && count == 0) {
                                attributes.put("value", Integer.parseInt(collarstyles.getJSONObject(0).get("value").toString()));
                                attributes.put("name", collarstyles.getJSONObject(0).get("name"));
                                attributes.put("diy_type", 0);
                                attributesArray.put(attributes);
                                commit = false;
                                count++;
...
//                                    }
                            ...
                            if (!matchFound && count == 0) {
                                attributes.put("value", Integer.parseInt(thicknesss.getJSONObject(0).get("value").toString()));
                                attributes.put("name", thicknesss.getJSONObject(0).get("name"));
                                attributes.put("diy_type", 0);
                                attributesArray.put(attributes);
                                commit = false;
                                count++;
                            ...
                            if (!matchFound && count == 0) {
                                attributes.put("value", Integer.parseInt(techniques.getJSONObject(0).get("value").toString()));
                                attributes.put("name", techniques.getJSONObject(0).get("name"));
                                attributes.put("diy_type", 0);
                                attributesArray.put(attributes);
                                commit = false;
                                count++;
                            ...
                            for (int j = 0; j < ingredientContents.length(); j++) {
                                ...
                                for (int k = 0; k < ulist.size(); k++) {
                                    //取得页面的属性字段
                                    String pageAttrName = (String) ulist.get(k);
                                    if (pageAttrName.contains(ingredientContent.getString("name").replace("及", "")) && count == 0) {
